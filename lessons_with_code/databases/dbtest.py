...
# make the function query2() return a list of Links submitted by user 62443, by
# submission time ascending
def query2():
    submitter_links = []
    for l in links:
        if l.submitter_id == 62443:
            submitter_links.append(l)
    #return sorted(submitter_links, key=lambda link: link.submitted_time)
    submitter_links.sort(key=lambda, link: link.submitted_time)
    return submitter_links
